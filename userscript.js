...
(function () {
  ...
  if (window.__mrpThreatHelperV20Loaded) {
    alert('MRP Threat Helper v20 already loaded');
    return;
  }
  ...
  const damageFeature = { enabled: false, hitCount: 0, baseTurbulence: null };
    ...
    try {
      if (damageFeature.hitCount >= 2) {
        if (damageFeature.baseTurbulence === null) {
          damageFeature.baseTurbulence = window.weather && window.weather.definition ? window.weather.definition.turbulences : 0;
        }
      ...
      console.warn('damage effect failed', e);
    }
  }
  ...
  function parseChatMessage(text, sender) {
    ...
    if (lock) {
      ...
      const threat = {
        phase: 'lock', sender: sender || 'Unknown', weapon, target: resolveName(target),
        ruleLabel: rule.label, counter: rule.counter, tactic: rule.tactic, raw: clean,
        time: Date.now(), statusText: 'LOCK ON YOU'
      };
    ...
    if (hm) {
      ...
      const finalThreat = {
        ...
        tactic: result === 'hit' ? 'Impact recorded.' : 'Miss recorded.',
        raw: clean,
        time: Date.now(),
  ...
  function buildPanel() {
    ...
    panel.style.cssText = 'position:fixed;top:18px;right:18px;z-index:99999;width:370px;max-width:calc(100vw - 20px);background:rgba(9,12,18,.93);color:#edf4ff;border:1px solid rgba(110,164,255,.33);border-radius:12px;box-shadow:0 14px 34px rgba(0,0,0,.38);font:12px/1.45 Arial,sans-serif;backdrop-filter:blur(8px);user-select:none';
  ...
  function render() {
    ...
    if (state.settingsOpen) {
      ...
      ui.body.querySelector('#mrp-save').onclick = () => {
        ...
        CONFIG.customWeaponAliases = ui.body.querySelector('#mrp-aliases').value;
...
'';
    ...
    ui.body.innerHTML =
      ...
      (CONFIG.showCountermeasures
        ? '<div style="margin-top:10px;padding:8px 9px;border:1px solid rgba(255,255,255,.09);border-radius:9px;background:rgba(255,255,255,.03)"><div style="font-size:11px;color:#9db0d3;margin-bottom:4px">Tactic</div><div>' + escapeHtml(t.tactic || '') + '</div></div>'
        : '') +
      '<div style="margin-top:10px;font-size:11px;color:#9db0d3">Recent outcomes</div>' +
      '<div style="margin-top:4px;max-height:150px;overflow:auto;padding-right:4px">' + (historyHtml || '<div style="color:#8ca0c7">No completed shots yet.</div>') + '</div>' +
